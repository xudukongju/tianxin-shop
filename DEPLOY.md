...
> **关于「报纸 / 每日时事」的运行条件**：报纸的实时新闻由服务端 `/api/news` 代理抓取（知乎日报 / BBC 中文）。因为这两个源**不允许浏览器直接跨域请求（CORS）**，所以报纸真实时事**必须有后端**，前端无法自己抓。
> - 部署到 Render（或本地 `node --experimental-sqlite server/sync-server.js`）打开 → 报纸是**真实每日新闻** ✅。
> - 纯静态方式打开（直接双击 `index.html`，或挂到只支持静态文件的平台）→ 没有后端，会按以下顺序退化：① 若浏览器此前在该后端地址打开过并缓存了真实新闻，则展示**上一次缓存的真实报纸**（顶部标「上次缓存的报纸」）；② 否则展示**本地精选文摘**（页面诚实标注「离线文摘」）。
> - 报纸页刊头右上角有 **↻ 刷新按钮**，联网时一键重新拉取。
> 结论：想稳定看到真实时事，直接用 Render 地址（或本地服务）打开即可，**无需为报纸做任何额外配置**。
